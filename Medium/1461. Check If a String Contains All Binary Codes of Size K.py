...
def hasAllCodes(s, k):
    ...
    :rtype: bool
    """
    #we can use the sliding window technique to find all substrings of length kin s.
    #furthermore, if we put them into a set, we can find all the unique substrings
    #with this, if the len(set) == 2**k: we know that all binary codes of size k are in the set

    uniqueCodes = set()
    #set([u'011', u'100', u'101', u'111', u'110'])
    i = 0
    while i <= len(s) - k:
        substring = s[i:i+k]
